"""Advent of Code 2021 Problem 08 Code Solution."""
...
def get_signal_patterns(file: str) -> list:
    """Return a list of signal patterns defined in `file`."""
...
def filter_table_with_segments(segments_table, signal_pattern, digit):
    """Filter `segments_table` to narrow down segment possibilities."""
    for segment in digit:
        segments_table[segment] = segments_table[segment] & set(signal_pattern)


def filter_final(segments_table: dict):
    """Filter `segments_table` to determine the signal patterns and digits."""
...
def default_table() -> dict:
    """Return a default table of segment labels to all possibilities."""
    SEGMENT_LABELS = range(SEGMENT_COUNT)
    all_possibilities_for_each_segment = [
        set("abcdefg") for _ in range(SEGMENT_COUNT)
    ]
    return dict(zip(SEGMENT_LABELS, all_possibilities_for_each_segment))


def decode_table(signals: tuple):
    """Return a decode table based on `signals`."""
    ...
    for pattern in signals:
        ...
        match segment_count:
            case 2:
                filter_table_with_segments(segments_table, pattern, DIGIT_1)
            case 4:
                filter_table_with_segments(segments_table, pattern, DIGIT_4)
            case 3:
                filter_table_with_segments(segments_table, pattern, DIGIT_7)
            case 7:
                filter_table_with_segments(segments_table, pattern, DIGIT_8)
            case 6:
...
def count_1_4_7_8_only(signal_patterns):
    """Return a dict with the count of 1, 4, 7, and 8 in `signal_patterns`."""
...
def get_digit(table: dict, pattern: str) -> int:
    """Decode pattern into a digit based on `table`."""
    segments = set()
...
def decode_patterns(signal_patterns):
    """Return a list of numbers decoded from the signal patterns."""
    numbers = []
...
def part_one_answer() -> int:
    ...
    _count = count_1_4_7_8_only(__signal_patterns)
    return sum(_count.values())


def part_two_answer() -> int:
    """Return part two answer."""
    _numbers = decode_patterns(__signal_patterns)
    return sum(_numbers)


__signal_patterns = get_signal_patterns("08/input.txt")
